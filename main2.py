...
class MainApp(CTk):
    def __init__(self) -> None:
        ...
        self.brinell_button = CTkButton(self, text="Ensayo Brinell", command=self.mostrar_ensayo_b, corner_radius=100, fg_color=(
            "#7CC0A2", "#45B584"), text_color="#000000", hover_color="#88D4B2", width=150)
        ...
        self.author = CTkLabel(self, text="Por Juan Carlos Alonso Luengo", fg_color=(
            "#D7D7D7", "#555555"), corner_radius=100)
        self.tema = CTkButton(self, image=image, text="Tema",  command=cambiar_apariencia, fg_color=(
            "#353D3A", "#A4A7A6"), text_color="#FFFFFF", hover_color=("#5A6863", "#787878"), width=150, corner_radius=100)

        # WIDGETS DE LAS PANTALLAS DE LOS ENSAYOS

        # Etiquetas de texto que mostrarán los resultados de los ensayos
        self.result1 = CTkLabel(self, width=150, fg_color=(
            "#D7D7D7", "#555555"), corner_radius=100)
        self.result2 = CTkLabel(self, width=150, fg_color=(
            "#D7D7D7", "#555555"), corner_radius=100)
        self.result3 = CTkLabel(self, width=150, fg_color=(
            "#D7D7D7", "#555555"), corner_radius=100)
        self.result4 = CTkLabel(self, width=150, fg_color=(
            "#D7D7D7", "#555555"), corner_radius=100)
        self.result5 = CTkLabel(self, width=150, fg_color=(
            "#D7D7D7", "#555555"), corner_radius=100)

        # Botones con diversas funciones. 1 - Calcula el ensayo 2- Cambia la apariencia 3 - Vuelve a la pantalla inicial
        self.button = CTkButton(self, text="Calcular ensayo", corner_radius=100, fg_color=("#7CC0A2", "#45B584"), text_color="#000000",
                                hover_color="#88D4B2", width=150)
        self.tema_e = CTkButton(self, image=image, text="Tema", command=cambiar_apariencia, corner_radius=100, fg_color=("#7CC0A2", "#45B584"),
                                text_color="#000000", hover_color="#88D4B2", width=5)
        self.back = CTkButton(self, text="Volver", command=self.mostrar_principal, corner_radius=100, fg_color=("#7CC0A2", "#45B584"),
                              text_color="#000000", hover_color="#88D4B2", width=5)
        ...
        self.datos = CTkLabel(self, text="Datos", width=150, fg_color=(
            "#7CC0A2", "#45B584"), corner_radius=100)
        self.res = CTkLabel(self, text="Resultados", width=150, fg_color=(
            "#7CC0A2", "#45B584"), corner_radius=100)
        self.error = CTkLabel(
            self, text="Has de rellenar al menos dos variables", text_color=("#7CC0A2", "#45B584"))

        self.type = IntVar(value=0)
        self.tension_checkbox = CTkRadioButton(
            self, text="Tensión", command=self.color_change, variable=self.type, value=1, fg_color=("#7CC0A2", "#45B584"))
        self.force_checkbox = CTkRadioButton(
            self, text="Fuerza", command=self.color_change, variable=self.type, value=2, fg_color=("#7CC0A2", "#45B584"))
        self.elong_checkbox = CTkRadioButton(
            self, text="Deformación", command=self.color_change, variable=self.type, value=3, fg_color=("#7CC0A2", "#45B584"))
        self.area_checkbox = CTkRadioButton(
            self, text="Area", command=self.color_change, variable=self.type, value=4, fg_color=("#7CC0A2", "#45B584"))
        self.const_checkbox = CTkRadioButton(
            self, text="Constante", command=self.color_change, variable=self.type, value=5, fg_color=("#7CC0A2", "#45B584"))

        self.proportional_zone = StringVar(value="off")
        self.proportional_checkbox = CTkCheckBox(self, text="Zona proporcional", command=self.color_change, variable=self.proportional_zone,
    ...
    def mostrar_principal(self):
        ...
        self.author.grid(column=0, row=2, padx=20, pady=20,
    ...
    def ocultar_principal(self):
        principal_list = [self.tema, self.brinell_button,
                          self.vickers_button, self.traction_button, self.name, self.author]
    ...
    def ocultar_ensayo(self):
        global ensayo_b, ensayo_v, ensayo_t
        if ensayo_b:
            ensayo_b_list = [self.entry1, self.entry2, self.entry3, self.entry4, self.entry5, self.datos, self.res, self.error, self.button,
                             self.result1, self.result2, self.result3, self.result4, self.result5, self.back, self.tema_e]
            for i in ensayo_b_list:
                i.grid_forget()
            ensayo_b_list_delete = [self.entry1, self.entry2,
                                    self.entry3, self.entry4, self.entry5]
            for i in ensayo_b_list_delete:
                i.delete(0, END)
            ensayo_b = False
        elif ensayo_v:
            ensayo_v_list = [self.entry1, self.entry2, self.entry3, self.entry4, self.button, self.datos, self.res, self.error,
                             self.result1, self.result2, self.result4, self.tema_e, self.back]
            for i in ensayo_v_list:
                i.grid_forget()

            ensayo_v_list_delete = [self.entry1, self.entry2, self.entry4]
            for i in ensayo_v_list_delete:
                i.delete(0, END)
            ensayo_v = False
        elif ensayo_t:
            ensayo_t_list = [self.button, self.tension_checkbox, self.force_checkbox, self.elong_checkbox, self.area_checkbox, self.const_checkbox, self.proportional_checkbox,
                             self.entry1, self.entry2, self.entry3, self.entry4, self.entry5, self.entry6, self.entry7, self.result1, self.back, self.tema_e]
            for i in ensayo_t_list:
                i.grid_forget()
            ensayo_t_list_delete = [self.entry1, self.entry2, self.entry3,
    ...
    def calcular_ensayo_v(self):
        ...
        self.result_value = float((self.entry1.get()).replace(
            ",", ".")) if self.entry1.get() else None
        self.force_value = float(self.entry2.get().replace(
            ",", ".")) if self.entry2.get() else None
        self.diameter_value_1 = float(self.entry3.get().replace(
            ",", ".")) if self.entry3.get() else None
        self.diameter_value_2 = float(self.entry4.get().replace(
            ",", ".")) if self.entry4.get() else None
    ...
    def calcular_ensayo_b(self):
        ...
        self.force_value = float(self.entry2.get().replace(
            ",", ".")) if self.entry2.get() else None
        self.diameter_value = float(self.entry3.get().replace(
            ",", ".")) if self.entry3.get() else None
        self.indentation_diameter_value = float(
            self.entry4.get().replace(",", ".")) if self.entry4.get() else None
        self.hardness_constant_value = float(
            self.entry5.get().replace(",", ".")) if self.entry5.get() else None
        ...
        self.result3.configure(
            text=f"{brinell_instance.indentation_diameter} mm ")
        self.result4.configure(text=f"{brinell_instance.force} kp ")
        self.result5.configure(
            text='El ensayo es fiable' if brinell_instance.fiability else 'El ensayo no es fiable')

    def calcular_ensayo_t(self):
        # Verificar si los CTkEntry están vacíos y asignar None si es el caso
        const_value = float(self.entry1.get().replace(
            ",", ".")) if self.entry1.get() else None
        force_value = float(self.entry2.get().replace(
            ",", ".")) if self.entry2.get() else None
        long1_value = float(self.entry4.get().replace(
            ",", ".")) if self.entry4.get() else None
        tension_value = float(self.entry3.get().replace(
            ",", ".")) if self.entry3.get() else None
        long2_value = float(self.entry5.get().replace(
            ",", ".")) if self.entry5.get() else None
        area_value = float(self.entry6.get().replace(
            ",", ".")) if self.entry6.get() else None
        elong_value = float(self.entry7.get().replace(
            ",", ".")) if self.entry7.get() else None

        # Pasar los valores a la función valores
        trac_valores(traction_instance, const=const_value, force=force_value, tension=tension_value,
                     long1=long1_value, long2=long2_value, area=area_value, elong=elong_value)

        # Llamar a la función ensayo, obtener el resultado y dejarlo en una variable
        result_trac = trac_ensayo(traction_instance, proportional=True if self.proportional_zone.get() == 'on' else False,
                                  tens=True if self.type.get() == 1 else False, const=True if self.type.get() == 5 else False,
                                  force=True if self.type.get() == 2 else False, elong=True if self.type.get() == 3 else False,
                                  area=True if self.type.get() == 4 else False)
        # Actualizar el widget result con el resultado de la función ensayo
        self.result1.configure(text=f"{result_trac} ")

    def color_change(self):
        entry_list = [self.entry1, self.entry2, self.entry3,
                      self.entry4, self.entry5, self.entry6, self.entry7]
        for i in entry_list:
            i.configure(fg_color="#f0f0f0")
        if self.type.get() == 1 and self.proportional_zone.get() == "on":
            for i in [self.entry1, self.entry7]:
                i.configure(fg_color=(
                    "#A1E2C6", "#A1E2C6"))
        elif self.type.get() == 1 and self.proportional_zone.get() == "off":
            for i in [self.entry2, self.entry6]:
                i.configure(fg_color=(
                    "#A1E2C6", "#A1E2C6"))
        elif self.type.get() == 2 and self.proportional_zone.get() == "off":
            for i in [self.entry6, self.entry3]:
                i.configure(fg_color=(
                    "#A1E2C6", "#A1E2C6"))
        elif self.type.get() == 2 and self.proportional_zone.get() == "on":
            self.proportional_checkbox.toggle()
            for i in [self.entry3, self.entry6]:
                i.configure(fg_color=(
                    "#A1E2C6", "#A1E2C6"))
        elif self.type.get() == 3 and self.proportional_zone.get() == "on":
            for i in [self.entry1, self.entry3]:
                i.configure(fg_color=(
                    "#A1E2C6", "#A1E2C6"))
        elif self.type.get() == 3 and self.proportional_zone.get() == "off":
            for i in [self.entry4, self.entry5]:
                i.configure(fg_color=(
                    "#A1E2C6", "#A1E2C6"))
        elif self.type.get() == 4 and self.proportional_zone.get() == "off":
            for i in [self.entry2, self.entry3]:
                i.configure(fg_color=(
                    "#A1E2C6", "#A1E2C6"))
            ...
            for i in [self.entry3, self.entry7]:
                i.configure(fg_color=(
                    "#A1E2C6", "#A1E2C6"))
